...
docker build -t secops-pipeline .

# Build with a specific tag
docker build -t secops-pipeline:v1.0 .

# Build without cache (fresh build)
docker build --no-cache -t secops-pipeline .

# Run the container
docker run -p 3000:80 secops-pipeline

# Run in detached mode (background)
docker run -d -p 3000:80 secops-pipeline

# Run with a custom name
docker run -d -p 3000:80 --name my-app secops-pipeline
```
...
# View image details
docker inspect secops-pipeline
```
...
# 2. Run the container
docker run -d -p 3000:80 --name autocraft secops-pipeline
...
docker build --build-arg NODE_VERSION=18 -t secops-pipeline .

# Run with environment variables
docker run -e NODE_ENV=production -p 3000:80 secops-pipeline

# Run with volume mount (for development)
docker run -v $(pwd):/app -p 3000:80 secops-pipeline

# Export container as tar
docker export <container-id> > backup.tar

# Save image as tar
docker save secops-pipeline > image.tar

# Load image from tar
docker load < image.tar

# Tag an image for registry
docker tag secops-pipeline:latest username/secops-pipeline:latest

# Push to Docker Hub
docker push username/secops-pipeline:latest
```
...
# Combine commands
docker build -t secops-pipeline . && docker run -d -p 3000:80 secops-pipeline
